...
while (false !== ($userData = $stmt->fetch())) {
    $signature = preg_replace('#[/=+]#', '',
        base64_encode(md5(sprintf('(57f-%s-gh-%s-yy', $userData['user_id'], $userData['user_code']) . $salt, true)) .
        base64_encode(md5(sprintf('%s9s%swl', $userData['user_id'], $userData['user_code']) . $salt, true))
    );

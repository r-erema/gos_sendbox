...
class Zend_Mime
{
    ...
    )
    {
        ...
        $i = 0;
	    while (strlen($str) > 0 && $i < 100) {
			echo "=={$i} iteration==" . PHP_EOL;
			$curr = microtime(true);
            $ptr = strlen($str);
	        $next = microtime(true);
			echo "strlen(result = {$ptr}): " . (($next - $curr) * 1000) . ' милисек. ' . PHP_EOL;
	        ...
	        $next = microtime(true);
			echo "strrpos(substr()): " . (($next - $curr) * 1000) . ' милисек. ' . PHP_EOL;
	        ...
	        $next = microtime(true);
	        echo "substr(): " . (($next - $curr) * 1000) . ' милисек. ' . PHP_EOL;
...
$curr = microtime(true);
strlen($html);
echo 'strlen(): ' . (microtime(true) - $curr) * 1000  . ' милисек.' . PHP_EOL;

$curr = microtime(true);
mb_strlen($html);
echo 'mb_strlen(): ' . (microtime(true) - $curr) * 1000 . ' милисек.' . PHP_EOL;


///Zend_Mime::encodeQuotedPrintable($html);
echo 'finish: ' . date('Y-m-d H:i:s') . PHP_EOL;
echo 'memory: ' . (memory_get_peak_usage() / 1024 / 1024) . PHP_EOL;
echo 'memory(real): ' . (memory_get_peak_usage(true) / 1024 / 1024) . PHP_EOL;
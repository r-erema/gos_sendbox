...
/**
  Original script from https://www.amazon.com/PHP-Beyond-Web-Rob-Aley/dp/1484224809
 */
 ...
 */
if ($pid === -1) {
    exit('Couldn\'t fork the child process');
}
...
/**
  As the parent has exited above, the following code is now executed solely by the child process.
 */
...
$pid = pcntl_fork();
if ($pid === -1) {
    exit('Couldn\'t fork the child process into grandchild');
}
...
/**
  ...
  would end badly. So instead we will re-create these three streams, but
  ...
  are called and wherever they point to). Thus the following also acts
...
/**
  ...
  We set a variable that will allow us to escape from the loop if we
...
while ($stayInLoop) {
     ...
     */
    shell_exec("notify-send 'Hello, The daemon is alive!'");
...
/**
  We've exited our loop, so do any cleanup required here
 */
shell_exec("notify-send 'The Daemon is now finished. Bye Bye.!'");
exit(0);
